...
void Conn_handler::create_room(json req_data)
{
    ...
    if (success_game_creation)
    {
        ...
        std::cout <<"GAME after create: size : " << game->game_map.size() << std::endl;
    }
    else
    {
...
void Conn_handler::get_valid_moves(json req_data)
{
    ...
    else
    {
        ...
        std::set<std::pair<int, int>> valid_moves = board->get_valid_moves(req_data["player_id"], req_data["piece_id"]);
        
        if(board->get_under_check(req_data["player_id"]) == true)
        {
            //Now that we have calculated all possible moves
            ...
            {
                if (board->if_move_remove_check(move, req_data["player_id"], req_data["piece_id"]) == true)
                {
                    //okay
                }
                else
                {
                    valid_moves.erase(move);//remove this move
                }
            }
        }
...
void Conn_handler::update_board(json req_data)
{
    ...
    //Note :- 
    /*
    * When the Player is in check this event only will be called
    * Before updating the board, check if the king's check will be removed
    * If the move leads to check removal update position and send confirmation to node to send to client
    * If not send not allowed to node to send to client
    */
    //code should be added for if move is made while king is under check
    //Code starts
    // 
    //uncomment after friday
    // 
    //if king is under check then move is allowed only if it stops the attack
...
//first called when player changes turn in node
//
void Conn_handler::validate_check(json req_data)
{
    std::cout << "inside vald check";
    Board* board = (*game)[req_data["room_id"]];
    json res_data = {};
    ...
    else
    {
        ...
        {
            //update the board under_check property of this player
